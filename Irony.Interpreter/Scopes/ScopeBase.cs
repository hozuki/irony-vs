...
namespace Irony.Interpreter {
    ...
    public abstract class ScopeBase {
        ...
        protected ScopeBase(ScopeInfo scopeInfo)
            : this(scopeInfo, null) {
        }

        protected ScopeBase(ScopeInfo scopeInfo, object[] values) {
            Info = scopeInfo;
            Values = values ?? new object[scopeInfo.ValuesCount];
        }

        public ScopeInfo Info { get; }

        internal volatile object[] Values;
...
#pragma warning disable 0420
        private void Resize(int newSize) {
            lock (Info.LockObject) {
                ...
                var tmp = Interlocked.Exchange(ref Values, null);
