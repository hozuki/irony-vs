...
namespace Irony.Tests {
    ...
    [TestClass]
    public sealed class DataLiteralsTests {

        [TestMethod]
        public void TestDataLiterals() {
            // FixedLengthLiteral ---------------------------------------------------------
            Terminal term = new FixedLengthLiteral("fixedLengthInteger", 2, TypeCode.Int32);
            var parser = TestHelper.CreateParser(term, null);

            var token = parser.ParseInput("1200");
            ...
            Assert.IsTrue(token?.Value != null, "Failed to parse fixed-length string.");
            ...
            Assert.IsTrue(token?.Value != null, "Failed to parse CSV integer.");
            ...
            Assert.IsTrue(token?.Value != null, "Failed to parse CSV string.");
            ...
            Assert.IsTrue(token?.Value != null, "Failed to parse quoted date.");
