...
";
        ...
        public void TestEvaluator_MemberAccess() {
            ...
            var script = @"foo.Field + ',' + foo.Prop + ',' + foo.GetStuff()";
            var result = eval.Evaluate(script);
...
R = foo.Field + foo.Prop ";
        ...
        public void TestEvaluator_ArrayDictDataRow() {
            ...
            eval.Globals["primes"] = new[] { 3, 5, 7, 11, 13 };
...
row['Name'] + ', ' + row['age']";
        ...
        private static string SayHello(ScriptThread thread, object[] args) {
            return "Hello, " + args[0] + "!";
        }
