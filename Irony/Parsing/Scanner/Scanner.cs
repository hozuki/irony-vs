...
namespace Irony.Parsing {
    ...
    public sealed class Scanner {
        ...
        private ParsingContext Context => Parser.Context;
        #endregion
        ...
        private void NextToken() {
            ...
            if (Context.Source.EOF) {
                var token = new Token(_grammar.Eof, Context.Source.Location, string.Empty, _grammar.Eof.Name); ;
        ...
        private bool MatchNonGrammarTerminals() {
            if (!Data.NonGrammarTerminalsLookup.TryGetValue(Context.Source.PreviewChar, out TerminalList terms)) {
                return false;
            }
        ...
        private void ComputeCurrentTerminals() {
            Context.CurrentTerminals.Clear();
            if (!Data.TerminalsLookup.TryGetValue(Context.Source.PreviewChar, out TerminalList termsForCurrentChar)) {
        ...
        public void VsSetSource(string text, int offset) {
            var line = Context.Source?.Location.Line ?? 0;
