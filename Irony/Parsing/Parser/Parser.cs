...
namespace Irony.Parsing {
    ...
    public sealed class Parser {
        ...
        internal ParserState InitialState { get; }
        ...
        public ParseTree Parse(string sourceText, string fileName) {
            ...
            Reset();
            var context = Context;
            context.Source = new SourceStream(sourceText, Language.Grammar.CaseSensitive, context.TabWidth, loc);
            ...
            if (parseTree.ParserMessages.Count > 0) {
                parseTree.ParserMessages.Sort(LogMessage.CompareByLocation);
            }
            return parseTree;
        }
        ...
        internal ParserAction GetNextAction() {
            ...
            var inputToken = currInput.Token;
            if (inputToken?.KeyTerm != null) {
        ...
        public void RecoverFromError() {
            Data.ErrorAction.Execute(Context);
        }
        #endregion

        #region Utilities
        private bool CheckPartialInputCompleted() {
            var partialCompleted = Context.Mode == ParseMode.CommandLine && Context.CurrentParserInput.Term == _grammar.Eof;
            if (!partialCompleted) {
                return false;
            }
