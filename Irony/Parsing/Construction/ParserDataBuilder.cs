...
namespace Irony.Parsing {
    ...
    internal sealed class ParserDataBuilder {
        ...
        private ParserState CreateInitialState(NonTerminal augmentedRoot) {
            ...
            var iniItemSet = new LR0ItemSet {
                augmentedRoot.Productions[0].LR0Items[0]
            };
        ...
        private void ExpandParserStateList(int initialIndex) {
            ...
            for (var index = initialIndex; index < _data.States.Count; index++) {
        ...
        private ParserState FindOrCreateState(LR0ItemSet coreItems) {
            var key = ComputeLR0ItemSetKey(coreItems);
            if (_stateHash.TryGetValue(key, out ParserState state)) {
                return state;
            }
            //create new state
        ...
        private static string ComputeLR0ItemSetKey(LR0ItemSet items) {
            if (items.Count == 0) return "";
            //Copy non-initial items to separate list, and then sort it
            var itemList = new LR0ItemList();
        ...
        private static int CompareLR0Items(LR0Item x, LR0Item y) => x.ID.CompareTo(y.ID);
        #endregion
        ...
        private readonly ParserStateHash _stateHash = new ParserStateHash();
