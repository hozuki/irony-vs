...
namespace Irony.Parsing {
    ...
    internal class GrammarDataBuilder {
        ...
        private void CollectTermsFromGrammar() {
            ...
            foreach (var t in _grammarData.Grammar.NonGrammarTerminals) {
        ...
        private void InitTermLists() {
            ...
            foreach (var term in _grammarData.AllTerms) {
                ...
                if (term is NonTerminal) {
                    _grammarData.NonTerminals.Add((NonTerminal)term);
                }
                if (term is Terminal && term != empty) {
                    _grammarData.Terminals.Add((Terminal)term);
                }
            }
        ...
        private static int CountNonPunctuationTerms(Production production) {
            return production.RValues.Count(rvalue => !rvalue.Flags.IsSet(TermFlags.IsPunctuation));
        }
        #endregion
        ...
        private int _lastItemId; //each LR0Item gets its unique ID, last assigned (max) Id is kept in this field
